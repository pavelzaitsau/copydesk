...
// for the cases a tool call identifies exactly.
//
...
// A Vale style lives under styles/<Style>/<Rule>.yml next to a .vale.ini.
...
const RULES = [
  ...
  },
  {
    ...
    applies: (tool, input) =>
      tool === 'Bash' &&
      /(^|[|&;\s])git\s+(checkout\s+-[bB]|switch\s+-[cC]|branch\s+-[mM]|worktree\s+add)(\s|$)/.test(
        input.command || ''
      ),
    context:
      'A branch is being created or renamed. Invoke the `branch-names` skill and name it ' +
      '`<type>/[<ticket-id>-]<slug>`: lower case, one slash, under 40 characters. ' +
      '`branch-names/scripts/branch-name.sh` builds the name and checks one.',
  },
  {
    id: 'commit',
    applies: (tool, input) =>
